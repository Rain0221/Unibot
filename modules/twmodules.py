...
def twaliastomusicid(alias):
    ...
    for row in cursor:
        with open('../twapi/masterdata/musics.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        name = ''
        for musics in data:
            if musics['id'] != row[1]:
    ...
    conn.close()
    return twmatchname(alias)


def twmatchname(alias):
...
def twsk(targetid=None, targetrank=None, secret=False):
    ...
    try:
        ...
        with open('../twapi/masterdata/cheerfulCarnivalTeams.json', 'r', encoding='utf-8') as f:
            Teams = json.load(f)
        with open('yamls/translate.yaml', encoding='utf-8') as f:
...
class userprofile(object):
    ...
    def getprofile(self, userid):
        ...
        with open('../twapi/masterdata/musicDifficulties.json', 'r', encoding='utf-8') as f:
...
def twpjskjindu(userid, private=False, diff='master'):
    profile = userprofile()
    ...
    with open('../twapi/masterdata/cards.json', 'r', encoding='utf-8') as f:
        cards = json.load(f)
    try:
        assetbundleName = ''
        for i in cards:
...
def twpjskprofile(userid, private=False):
    profile = userprofile()
    profile.getprofile(userid)
    if private:
        id = '保密'
...
def generatehonor(honor, ismain=True):
    ...
    if honor['profileHonorType'] == 'normal':
        # 普通牌子
        with open('../twapi/masterdata/honors.json', 'r', encoding='utf-8') as f:
            honors = json.load(f)
        with open('../twapi/masterdata/honorGroups.json', 'r', encoding='utf-8') as f:
        ...
        # cp牌子
        with open('../twapi/masterdata/bondsHonors.json', 'r', encoding='utf-8') as f:
...
def twpjskb30(userid, private=False):
    resp = requests.get(f'{apiurl}/user/{userid}/profile')
    data = json.loads(resp.content)
    # with open('piccache\profile.json', 'r', encoding='utf-8') as f:
    #     data = json.load(f)
...
def twdrawpjskinfo(musicid):
    info = musicinfo()

    with open(r'../twapi/masterdata/musics.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    for music in data:
        if music['id'] != musicid:
            continue
    ...
    with open(r'../twapi/masterdata/musicDifficulties.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    for i in range(0, len(data)):
...
def twbindid(qqnum, userid):
    ...
    for raw in cursor:
        alreadyin = True
    if alreadyin:
        c.execute(f'UPDATE twbind SET userid=? WHERE qqnum=?', (userid, qqnum))
    else:
        sql_add = 'insert into twbind(qqnum,userid,isprivate) values(?, ?, ?)'
        c.execute(sql_add, (str(qqnum), str(userid), 0))
    conn.commit()
    conn.close()
    return "绑定成功！"

def twsetprivate(qqnum, isprivate):
    conn = sqlite3.connect('pjsk.db')
    c = conn.cursor()
    cursor = c.execute(f'SELECT * from twbind where qqnum=?', (qqnum,))
    alreadyin = False
    for raw in cursor:
        alreadyin = True
    if alreadyin:
        c.execute(f'UPDATE twbind SET isprivate=? WHERE qqnum=?', (isprivate, qqnum))
    else:
        conn.close()
        return False
    conn.commit()
